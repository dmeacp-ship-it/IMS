...
async function resolveDestination(session, transactionId, branchCode) {
  ...
  const { data: trans } = await supabase
    ...
    .eq('id', transactionId)
    .limit(1);
  const docnum = trans && trans[0] ? trans[0].docnum : transactionId;
  ...
  await logActivity(session.username, 'Manual Tagging', 'Tagged transfer docnum ' + docnum + ' to ' + branchCode);

  return { success: true };
}

async function resolveDestinationsBulk(session, transactionIds, branchCode) {
  if (!transactionIds || !Array.isArray(transactionIds) || transactionIds.length === 0 || !branchCode) {
    throw httpError(400, 'Missing transaction IDs or destination branch code.');
  }
  ...
  if (error) throw httpError(500, 'Failed to update transactions: ' + error.message);

  await logActivity(session.username, 'Bulk Tagging', 'Bulk tagged ' + transactionIds.length + ' transfers to branch ' + branchCode);
...
async function _fetchLedgerPaged(applyFilters, columns) {
  columns = columns || '*';
  const PAGE = 1000;
  const CONCURRENCY = 5;

  let countQ = supabase.from('item_stock_ledger').select('branch_code', { count: 'exact', head: true });
  ...
  async function worker() {
    while (next < numPages) {
      const p = next++;
      let q = supabase.from('item_stock_ledger').select(columns)
...
async function getItemLedger(branchCode) {
  const rows = await _fetchLedgerPaged(function (q) { return q.eq('branch_code', branchCode); });
  return mergeInTransitIntoLedger(rows, [branchCode]);
}

async function getAllItemLedger() {
  const rows = await _fetchLedgerPaged(function (q) { return q; });
...
async function getHodItemLedger(session) {
  ...
  const rows = await _fetchLedgerPaged(function (q) { return q.in('branch_code', branchCodes); }, '*');
...
async function syncGoogleSheet(triggeredBy, mode) {
  ...
  try {
    ...
    for (let i = 1; i < parsedRows.length; i++) {
      ...
      const isTransfer = VIRGO_ROOT_NAMES.some(function (root) { return customerName.toUpperCase().indexOf(root) !== -1; });
...
// Best-effort: if it can't be built, valuation is simply omitted (null).
async function _itemRateMap() {
  ...
  (data || []).forEach(function (r) {
    ...
    // First row per item wins — rows are ordered newest-first, so this is the
    // latest known rate for that item.
...
async function getVarianceReport(session) {
  ...
  // 2. Latest physical count per branch+item+batch (newest-first, first wins).
  ...
  if (recErr) throw httpError(500, 'Failed to load reconciliations: ' + recErr.message);
  ...
  // 3. Current ledger closing qty for the same scope.
  const ledger = await _fetchLedgerPaged(function (q) {
  ...
  // 4. Rate map for ₹ valuation.
  const rateMap = await _itemRateMap();
