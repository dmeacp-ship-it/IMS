...
async function getBranchSummary() {
  ...
  rows.forEach(r => {
    ...
    item.opening_qty += Number(r.opening_qty || 0);
    item.inward_qty += Number(r.inward_qty || 0);
    item.outward_qty += Number(r.outward_qty || 0);
    item.in_transit_qty += Number(r.in_transit_qty || 0);
    item.closing_qty += Number(r.closing_qty || 0);
