...
async function syncGoogleSheet(triggeredBy, mode, opts) {
  ...
  try {
    ...
    // Only wipe once, at the very start of the run — not on every slice.
    if (mode === 'HARD_RESET' && phase === 'transactions' && offset === 0) {
...
async function syncGoogleSheetAll(triggeredBy, mode, budgetMs) {
  const start = Date.now();
  let state = { phase: 'transactions', offset: 0, stats: {} };
  for (;;) {
    const step = await syncGoogleSheet(triggeredBy, mode, state);
    if (step.done) return step;
    state = { phase: step.phase, offset: step.nextOffset, stats: step.stats };
