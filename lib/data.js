...
async function markReceived(session, transactionId) {
  ...
  // Rebuild the all-branches snapshot so the received qty lands in inward there
  // too (the branch's own ledger already reads the live view). Best-effort.
  await refreshItemStockLedger();
...
async function bulkUpdateTransferStatus(session, rows) {
  ...
  // Fetch targeted transfer records matching the uploaded DOCNUMs in fast parallel/chunked queries
  const docnumArray = Array.from(wantedDocnums);
  const matchedTransfers = [];
  const CHUNK_SIZE = 200;
  for (let i = 0; i < docnumArray.length; i += CHUNK_SIZE) {
    const chunk = docnumArray.slice(i, i + CHUNK_SIZE);
    const { data, error } = await supabase
      .from('sales_transactions')
      .select('id, docnum, item_code, batch, status')
      .eq('order_type', 'BRANCH-TRANSFER')
      .in('docnum', chunk);
    if (error) throw httpError(500, 'Failed to query transfer records: ' + error.message);
    if (data && data.length) matchedTransfers.push.apply(matchedTransfers, data);
  }

  const curMap = {};
  matchedTransfers.forEach(function (t) {
    curMap[t.docnum + '|' + t.item_code + '|' + (t.batch || '').trim().toUpperCase()] = { id: t.id, status: t.status };
  });
  ...
  if (toReceived.length > 0 || toInTransit.length > 0) {
    // Await so the snapshot is rebuilt before we respond; on Vercel a
    // fire-and-forget promise can be frozen before it runs. refreshItemStockLedger
    // swallows its own errors, so a refresh hiccup never fails the status update.
    await refreshItemStockLedger();
  }
...
async function mergeInTransitIntoLedger(ledgerRows, allowedBranches, excludeBranches) {
  let query = supabase
    .from('sales_transactions')
    .select('destination_branch_code, item_description, batch, quantity')
    .eq('order_type', 'BRANCH-TRANSFER')
    .eq('status', 'IN_TRANSIT')
    .gte('doc_date', new Date(Date.now() - 45 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]);
  ...
  if (trans) {
    trans.forEach(function (t) {
      ...
      if (excludeBranches && excludeBranches.indexOf(branch) !== -1) return;
...
async function _fetchLedgerPaged(applyFilters, columns, source) {
  ...
  rows.sort((a, b) => {
    ...
    const qtyA = a.closing_qty || 0;
    const qtyB = b.closing_qty || 0;
...
// Facilities flagged as FACTORY produce stock, but production isn't modeled as
// inward — so their ledger runs deeply negative (the factory "ships" far more
// than it ever "received"). Per the product decision, factories are treated as
// transfer SOURCES only and excluded from the aggregate stock/planning views
// (Stock Ledger all-branches, Branch Summary, Order Planning). Cached briefly.
let _factoryCache = { codes: null, ts: 0 };
async function _factoryBranchCodes() {
  const now = Date.now();
  if (_factoryCache.codes && now - _factoryCache.ts < 5 * 60 * 1000) return _factoryCache.codes;
  const { data, error } = await supabase.from('branches').select('code').eq('facility_type', 'FACTORY');
  const codes = error ? [] : (data || []).map(function (b) { return b.code; });
  if (!error) _factoryCache = { codes: codes, ts: now };
  return codes;
}
...
async function getAllItemLedger() {
  ...
  // Factories are excluded (production isn't tracked, so their closing is
  // meaninglessly negative — see _factoryBranchCodes).
  const factories = await _factoryBranchCodes();
  const rows = await _fetchLedgerPaged(function (q) {
    return factories.length ? q.not('branch_code', 'in', '(' + factories.join(',') + ')') : q;
  }, LEDGER_COLS, 'item_stock_ledger_mat');
  return mergeInTransitIntoLedger(rows, null, factories);
}

// Internal, best-effort rebuild of the item_stock_ledger_mat snapshot after a
// write that changes stock (e.g. a transfer flipped to RECEIVED). Swallows its
// own errors — the caller's primary write has already committed, and pg_cron
// (patch_015) will catch up on the next cycle if this refresh hiccups. This is
// what the bulk transfer-status update and mark-received paths call so the
// all-branches ledger reflects the change immediately instead of showing stale
// inward = 0 until the next scheduled refresh.
async function refreshItemStockLedger() {
  const { error } = await supabase.rpc('refresh_item_stock_ledger');
  if (error) console.error('Ledger matview refresh failed: ' + error.message);
}
...
async function getBranchSummary() {
  const rows = await getAllItemLedger();
  const branchMap = {};
  ...
  rows.forEach(r => {
    ...
    if (!branchMap[code]) {
      branchMap[code] = {
        branch_code: code,
        opening_as_of_date: r.opening_as_of_date || null,
        opening_qty: 0,
        inward_qty: 0,
        outward_qty: 0,
        in_transit_qty: 0,
    ...
    const item = branchMap[code];

    const hasOpeningDate = !!r.opening_as_of_date;
    const openQty = Number(r.opening_qty || 0);
    const inward = hasOpeningDate ? Number(r.inward_qty || 0) : 0;
    const outward = hasOpeningDate ? Number(r.outward_qty || 0) : 0;
    const transit = Number(r.in_transit_qty || 0);
    const closing = hasOpeningDate ? Number(r.closing_qty || 0) : openQty;

    item.opening_qty += openQty;
    item.inward_qty += inward;
    item.outward_qty += outward;
    item.in_transit_qty += transit;
    item.closing_qty += closing;

    if (!item.opening_as_of_date && r.opening_as_of_date) {
      item.opening_as_of_date = r.opening_as_of_date;
    }
  });
...
async function getOrderPlanning(session) {
  ...
  const PAGE = 1000;
  const CONCURRENCY = 5;
  // For the all-branches (admin) view, exclude factories — production isn't
  // tracked, so their closing/current_stock is meaninglessly negative and would
  // pollute the planning worksheet (matches the Stock Ledger exclusion).
  const excludeFactories = scope ? [] : await _factoryBranchCodes();
  function applyScope(q) {
    if (scope) return q.in('branch_code', scope);
    return excludeFactories.length ? q.not('branch_code', 'in', '(' + excludeFactories.join(',') + ')') : q;
  }

  // Probe with a real (non-head) count so a MISSING op_planning relation surfaces
  // as a clear, actionable error. A head+count on a dropped matview returns a
  // null count with NO error (HTTP 204), which is indistinguishable from a
  // genuinely empty table — that is exactly what made the "op_planning was
  // cascade-dropped" failure render as a misleading "No planning data" screen.
  const probe = await applyScope(
    supabase.from('op_planning').select('branch_code', { count: 'exact' }).limit(1)
  );
  if (probe.error) {
    const m = probe.error.message || '';
    if (/(does not exist|schema cache|could not find)/i.test(m)) {
      throw httpError(503, 'Order Planning is not set up yet — the "op_planning" table is missing (it gets cascade-dropped when the ledger is rebuilt). Run supabase/patch_025_restore_op_planning.sql in the Supabase SQL editor, then click Refresh.');
    }
    throw httpError(500, 'Failed to load order planning: ' + m);
  }
  ...
  // Dynamically calculate IN_TRANSIT to match stock ledger exactly (with 45-day window)
  let transitQ = supabase
    .from('sales_transactions')
    .select('destination_branch_code, item_description, quantity')
    .eq('order_type', 'BRANCH-TRANSFER')
    .eq('status', 'IN_TRANSIT')
    .gte('doc_date', new Date(Date.now() - 45 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]);
  if (scope) transitQ = transitQ.in('destination_branch_code', scope);
  const { data: trans } = await transitQ;
  ...
  if (trans) {
    trans.forEach(t => {
      ...
      transitMap[key] = (transitMap[key] || 0) + Number(t.quantity || 0);
    });
  }

  rows.forEach(function (r) {
    const tKey = r.branch_code + '||' + r.item_name;
    r.in_transit = transitMap[tKey] || 0;
  });

  const familyTotals = {};
  rows.forEach(r => {
    const key = r.branch_code + '||' + (r.family || '') + '||' + (r.variant || '');
    familyTotals[key] = (familyTotals[key] || 0) + (r.branch_sales_qty || 0);
  });
  ...
  rows.sort((a, b) => {
    ...
    const keyA = a.branch_code + '||' + (a.family || '') + '||' + (a.variant || '');
    const keyB = b.branch_code + '||' + (b.family || '') + '||' + (b.variant || '');
    
    const diffFam = (familyTotals[keyB] || 0) - (familyTotals[keyA] || 0);
    if (diffFam !== 0) return diffFam;
    
    if (keyA < keyB) return -1;
    if (keyA > keyB) return 1;
    
    const diffQty = (b.branch_sales_qty || 0) - (a.branch_sales_qty || 0);
...
async function createConversion(session, entry) {
  ...
  // Explicitly refresh the materialized view after insertion
  await supabase.rpc('refresh_item_stock_ledger');
...
// Shared by BOTH sync phases (transactions and returns), so these live at module
// scope — when they were declared inside the transactions branch the returns
// phase blew up with "branchMap is not defined".
function _num(val) {
  const n = Number(val);
...
// Normalized sheet branch name → branch code.
async function _branchNameMap() {
  const { data, error } = await supabase.from('branches').select('code,name');
  if (error) throw httpError(500, 'Failed to fetch branch map: ' + error.message);
  const map = {};
  (data || []).forEach(function (b) { map[_normalize(b.name)] = b.code; });
  return map;
}

// Rows of the sales sheet pulled+upserted per call. The whole sheet (~70k rows)
// takes ~65s end-to-end, far past Vercel's function limit, so the sync is
// resumable: each call handles one slice and reports where to continue. The
// caller (Settings UI / cron) loops until done === true.
const SYNC_CHUNK = 10000;

// opts: { phase: 'transactions' | 'returns', offset: number, stats: {...} }
// Returns { done, phase, nextOffset, processed, stats } so the caller can drive
// the loop and show progress. Back-compat: called with no opts it starts at the
// beginning of the transactions phase.
async function syncGoogleSheet(triggeredBy, mode, opts) {
  opts = opts || {};
  const phase = opts.phase || 'transactions';
  const offset = Number(opts.offset) || 0;
  // Running totals threaded through the loop so the final log reports the whole
  // run, not just the last slice.
  const stats = Object.assign({ transactions: 0, returns: 0 }, opts.stats || {});
  ...
  try {
    ...
    // DESTRUCTIVE. Deleting here re-inserts every row fresh, which permanently
    // loses per-row state the sheet doesn't carry — above all the RECEIVED
    // transfer status (plus received_at/received_by). It is also unsafe when the
    // import is chunked: the wipe happens on the first slice, so an interrupted
    // run leaves the table holding only the slices that made it.
    //
    // It therefore requires an explicit opts.allowDestructive from a caller that
    // runs the whole import in one process (CLI). The HTTP endpoint never sets
    // it, so a browser sync is always a non-destructive upsert. To genuinely
    // start clean, use the password-protected "Clear All Data" danger zone and
    // then sync.
    if (mode === 'HARD_RESET' && opts.allowDestructive === true && phase === 'transactions' && offset === 0) {
    ...
    // Needed by both phases, so resolve it before the phase split.
    const branchMap = await _branchNameMap();

    if (phase === 'transactions') {
    // Pull only this slice. gviz applies limit/offset to DATA rows and repeats
    // the header on every slice, so each slice parses standalone.
    const tq = encodeURIComponent('select * limit ' + SYNC_CHUNK + ' offset ' + offset);
    const csvUrl = 'https://docs.google.com/spreadsheets/d/' + googleSpreadsheetId + '/gviz/tq?tqx=out:csv&sheet=' + encodeURIComponent(googleSheetName) + '&tq=' + tq;
    let response;
    try {
    ...
    if (parsedRows.length < 2) {
      // Header-only slice: past the last row. At offset 0 that means the sheet
      // is genuinely empty; otherwise the transactions phase is simply finished.
      if (offset === 0) {
        throw httpError(400, 'The sheet "' + googleSheetName + '" contains no rows to sync.');
      }
      return { done: false, phase: 'returns', nextOffset: 0, processed: 0, stats: stats };
    }
    ...
    for (let i = 1; i < parsedRows.length; i++) {
      ...
      let normBranch = _normalize(branchName);
      // Strip common CSV suffixes that don't exist in the database names
      normBranch = normBranch.replace(/MAIN$/, '').replace(/BRANCH$/, '');
      
      const sourceBranchCode = branchMap[normBranch] || branchMap[_normalize(branchName)] || null;
      if (!sourceBranchCode) {
    ...
    stats.transactions += upsertedCount;
    if (missingBranches.size > 0) {
      // Accumulate across slices so the final log lists them all, de-duped.
      const seen = new Set(stats.missingBranches || []);
      missingBranches.forEach(function (b) { seen.add(b); });
      stats.missingBranches = Array.from(seen);
    }

    // A short slice means we've reached the end of the sheet; otherwise resume
    // from the next offset on the following call.
    if ((parsedRows.length - 1) >= SYNC_CHUNK) {
      return { done: false, phase: 'transactions', nextOffset: offset + SYNC_CHUNK, processed: upsertedCount, stats: stats };
    }
    return { done: false, phase: 'returns', nextOffset: 0, processed: upsertedCount, stats: stats };
    } // ── end transactions phase; the returns phase runs on its own call ──
    ...
    // Totals are threaded through the whole chunked run, so the log reflects
    // every slice — not just this final call.
    let detailsStr = 'Successfully synced ' + stats.transactions + ' transactions and ' + stats.returns + ' sales returns.';
    if (returnsError) {
      detailsStr += ' WARNING: sales returns did not fully sync — ' + returnsError;
    }
    if (stats.missingBranches && stats.missingBranches.length > 0) {
      detailsStr += ' Skipped unrecognized branch names: ' + stats.missingBranches.join(', ');
    }

    await supabase.from('sync_logs').insert({
      triggered_by: triggeredBy,
      status: returnsError ? 'PARTIAL' : 'SUCCESS',
      synced_count: stats.transactions,
      details: detailsStr
    });

    // We no longer explicitly refresh the materialized view here because 
    // it can push the execution time over the 10s Vercel Serverless limit.
    // The pg_cron job will refresh it every 15 minutes, or the user can
    // manually click the "Refresh Ledger" button in the UI.

    return {
      success: true, done: true, phase: 'returns',
      synced: stats.transactions, returnsSynced: stats.returns,
      returnsError: returnsError, stats: stats
    };
  } catch (err) {
    await supabase.from('sync_logs').insert({
      triggered_by: triggeredBy,
...
// Drive the chunked sync to completion in one process. Used by the cron job and
// CLI, where there's no client to loop. `budgetMs` stops the loop early and
// returns a resume point instead of being killed mid-slice by the platform.
async function syncGoogleSheetAll(triggeredBy, mode, budgetMs, allowDestructive) {
  const start = Date.now();
  // Safe to honour a HARD_RESET here only because this driver runs every slice
  // in one process, so the wipe can't strand the table half-imported.
  let state = { phase: 'transactions', offset: 0, stats: {}, allowDestructive: allowDestructive === true };
  for (;;) {
    const step = await syncGoogleSheet(triggeredBy, mode, state);
    if (step.done) return step;
    state = { phase: step.phase, offset: step.nextOffset, stats: step.stats, allowDestructive: allowDestructive === true };
    if (budgetMs && (Date.now() - start) > budgetMs) {
      return { success: false, done: false, incomplete: true, resume: state, stats: step.stats };
    }
  }
}
