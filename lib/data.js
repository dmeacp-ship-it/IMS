...
async function getAllItemLedger() {
  ...
  const rows = await _fetchLedgerPaged(function (q) { return q; }, '*', 'item_stock_ledger_mat');
...
async function getVarianceReport(session) {
  ...
  // 3. Current ledger closing qty for the same scope. Read the precomputed
  //    matview — a variance report tolerates the small refresh lag and this
  //    keeps the all-branches case under the 10s function limit.
