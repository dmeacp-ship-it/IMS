...
async function getBranchDashboard(session) {
  ...
  const { data: trendData } = await supabase
    .from('sales_transactions')
    .select('doc_date, quantity')
    .eq('source_branch_code', branchCode)
    .gte('doc_date', thirtyDaysStr);

  const trendMap = {};
  if (trendData) {
    trendData.forEach(function (r) {
      if (!r.doc_date) return;
      const d = r.doc_date.split('T')[0];
      trendMap[d] = (trendMap[d] || 0) + (Number(r.quantity) || 0);
    });
  }
