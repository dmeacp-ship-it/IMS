...
async function logout() {
  try { localStorage.removeItem('ims_session'); } catch (e) {}
...
var _vsJobs = new WeakMap();

function paintTable(wrap, tableOpen, headHTML, rowStrings, emptyHTML) {
  var prev = _vsJobs.get(wrap);
  if (prev) { wrap.removeEventListener('scroll', prev); _vsJobs.delete(wrap); }

  if (!rowStrings || !rowStrings.length) {
    wrap.innerHTML = emptyHTML || emptyState('ph-magnifying-glass', 'No matching rows', 'Try a different search or filter.');
    return;
  }
  ...
  // Count columns for the spacer colspan
  var colCount = (headHTML.match(/<th/g) || []).length || 10;
  ...
  function renderViewport() {
    ...
    var html = '<tr style="height:' + topHeight + 'px; border:none; background:transparent;"><td colspan="' + colCount + '" style="padding:0; border:none;"></td></tr>';
    html += rowStrings.slice(startIdx, endIdx).join('');
    html += '<tr style="height:' + bottomHeight + 'px; border:none; background:transparent;"><td colspan="' + colCount + '" style="padding:0; border:none;"></td></tr>';
  ...
  _vsJobs.set(wrap, onScroll);
  
  // Force synchronous initial paint for visible rows to avoid blank flashes
  renderViewport();
}
...
async function boot() {
  ...
  var cached = null;
  try {
    var str = localStorage.getItem('ims_session');
  ...
  if (cached) {
    SESSION = cached;
    launchApp();
    apiGet('/api/session').then(function(s) {
      localStorage.setItem('ims_session', JSON.stringify(s));
      if (s.role !== SESSION.role || s.branchCode !== SESSION.branchCode || s.username !== SESSION.username) {
        window.location.reload();
      }
    }).catch(function() {
    ...
    try {
      ...
      if (SESSION) localStorage.setItem('ims_session', JSON.stringify(SESSION));
    } catch (e) {
      SESSION = null;
    }
    launchApp();
