-- ============================================================================
-- Virgo ACP IMS — patch_024_fix_branch_opening_date_cutoff.sql
-- Fixes an issue where sales transactions for items whose batch did not match
-- opening_stock were not being filtered by the branch's opening date cutoff,
-- causing pre-opening historical sales transactions to be summed into outward.
-- ============================================================================
...
with opening as (
  select upper(trim(branch_code)) as branch_code,
...
txn as (
  select
    upper(trim(source_branch_code)) as source_branch_code,
    upper(trim(destination_branch_code)) as destination_branch_code,
    order_type,
    status,
    doc_date,
...
outward as (
  ...
  where t.source_branch_code is not null
    and (
      (o.as_of_date is null and bo.branch_date is null)
...
conv_out as (
  select upper(trim(branch_code)) as branch_code, upper(trim(from_item_name)) as item_name, coalesce(upper(trim(from_batch)), '') as batch, sum(from_quantity) as qty
  from stock_conversions
  group by 1, 2, 3
),
conv_in as (
  select upper(trim(branch_code)) as branch_code, upper(trim(to_item_name)) as item_name, coalesce(upper(trim(to_batch)), '') as batch, sum(to_quantity) as qty
  from stock_conversions
  group by 1, 2, 3
),
adjustments as (
  select upper(trim(branch_code)) as branch_code, upper(trim(item_name)) as item_name, coalesce(upper(trim(batch)), '') as batch, sum(adjustment_qty) as qty
...
select
  ...
  coalesce(os.as_of_date, bo.branch_date) as opening_as_of_date
from keys k
left join opening         os  using (branch_code, item_name, batch)
left join branch_open     bo  using (branch_code)
