-- ============================================================================
-- Virgo ACP IMS — patch_013_opening_date_inclusive.sql
-- Adjusts the opening-date cut-off from STRICTLY-AFTER to ON-OR-AFTER.
--
-- Supersedes patch_012. There the cut-off was doc_date > as_of_date (opening =
-- end-of-day balance). Per updated requirement, transactions dated exactly ON
-- the opening date should also count as movement, so the cut-off is now
-- doc_date >= as_of_date (opening = start-of-day balance).
--
-- Everything else is identical to patch_012: only transaction-sourced flows
-- (outward, received-transfer inward, sales returns) are date-filtered; keys
-- with no opening count everything; conversions and adjustments are not
-- date-filtered.
--
-- Idempotent: safe to re-run. Run AFTER patch_012 (or instead of it — this
-- fully redefines the view).
-- ============================================================================
